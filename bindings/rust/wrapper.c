...
fid_t get_fid_ptr(void* ptr) { return (fid_t)ptr; }
...
struct fi_info* wrap_fi_allocinfo(void) { return fi_allocinfo(); }

int wrap_fi_close(struct fid* fid) { return fi_close(fid); }
...
int wrap_fi_enable(struct fid_ep* ep) { return fi_enable(ep); }
...
ssize_t wrap_fi_rx_size_left(struct fid_ep* ep) { return fi_rx_size_left(ep); }

ssize_t wrap_fi_tx_size_left(struct fid_ep* ep) { return fi_tx_size_left(ep); }
...
ssize_t wrap_fi_atomic(struct fid_ep* ep, const void* buf, size_t count,
                       ...
                       void* context) {
    return fi_atomic(ep, buf, count, desc, dest_addr, addr, key, datatype, op,
                     context);
}
...
ssize_t wrap_fi_atomicv(struct fid_ep* ep, const struct fi_ioc* iov,
    ...
    return fi_atomicv(ep, iov, desc, count, dest_addr, addr, key, datatype, op,
                      context);
}
...
ssize_t wrap_fi_fetch_atomic(struct fid_ep* ep, const void* buf, size_t count,
                             ...
                             enum fi_datatype datatype, enum fi_op op,
                             void* context) {
    return fi_fetch_atomic(ep, buf, count, desc, result, result_desc, dest_addr,
...
ssize_t wrap_fi_compare_atomicmsg(struct fid_ep* ep,
                                  ...
                                  struct fi_ioc* resultv, void** result_desc,
                                  size_t result_count, uint64_t flags) {
    return fi_compare_atomicmsg(ep, msg, comparev, compare_desc, compare_count,
...
int wrap_fi_mr_reg(struct fid_domain* domain, const void* buf, size_t len,
    ...
    return fi_mr_reg(domain, buf, len, acs, offset, requested_key, flags, mr,
                     context);
}
...
int wrap_fi_mr_regv(struct fid_domain* domain, const struct iovec* iov,
                    ...
                    void* context) {
    return fi_mr_regv(domain, iov, count, acs, offset, requested_key, flags, mr,
                      context);
}
...
void* wrap_fi_mr_desc(struct fid_mr* mr) { return fi_mr_desc(mr); }

uint64_t wrap_fi_mr_key(struct fid_mr* mr) { return fi_mr_key(mr); }
...
int wrap_fi_mr_enable(struct fid_mr* mr) { return fi_mr_enable(mr); }
...
int wrap_fi_av_insertsym(struct fid_av* av, const char* node, size_t nodecnt,
                         ...
                         uint64_t flags, void* context) {
    return fi_av_insertsym(av, node, nodecnt, service, svccnt, fi_addr, flags,
                           context);
}
...
int wrap_fi_av_insert_auth_key(struct fid_av* av, const void* auth_key,
                               ...
                               uint64_t flags) {
    return fi_av_insert_auth_key(av, auth_key, auth_key_size, fi_addr, flags);
}
...
int wrap_fi_listen(struct fid_pep* pep) { return fi_listen(pep); }
...
fi_addr_t wrap_fi_mc_addr(struct fid_mc* mc) { return fi_mc_addr(mc); }
...
ssize_t wrap_fi_broadcast(struct fid_ep* ep, void* buf, size_t count,
                          ...
                          void* context) {
    return fi_broadcast(ep, buf, count, desc, coll_addr, root_addr, datatype,
                        flags, context);
}
...
ssize_t wrap_fi_allreduce(struct fid_ep* ep, const void* buf, size_t count,
    ...
    return fi_allreduce(ep, buf, count, desc, result, result_desc, coll_addr,
                        datatype, op, flags, context);
}
...
ssize_t wrap_fi_allgather(struct fid_ep* ep, const void* buf, size_t count,
                          ...
                          fi_addr_t coll_addr, enum fi_datatype datatype,
                          uint64_t flags, void* context) {
    return fi_allgather(ep, buf, count, desc, result, result_desc, coll_addr,
                        datatype, flags, context);
}
...
int wrap_fi_cq_signal(struct fid_cq* cq) { return fi_cq_signal(cq); }
...
uint64_t wrap_fi_cntr_read(struct fid_cntr* cntr) { return fi_cntr_read(cntr); }
...
int wrap_fi_profile_register_callback(
    ...
    void* context) {
    return fi_profile_register_callback(prof_fid, event_id, callback, context);
}
