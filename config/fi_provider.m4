...
AC_DEFUN([FI_PROVIDER_FINI],[
	...
	constructors=
	destructors=
	for provider in $PROVIDERS_STATIC; do
		constructor_decls="void ${provider}_ini(void); $constructor_decls"
		constructor_list="${provider}_ini,$constructor_list"
		destructor_decls="void ${provider}_fini(void); $destructor_decls"
		destructor_list="${provider}_fini,$destructor_list"
	done

	file="$srcdir/include/fi_static_providers.h"
	rm -f "$file"
	cat > "$file" <<EOF
/*
 * libfabric static providers
 * This file is automatically generated -- edits will be lost!
 */

#ifndef FI_STATIC_PROVIDERS_H
#define FI_STATIC_PROVIDERS_H

typedef void (fi_prov_constructor_t)(void);
typedef void (fi_prov_destructor_t)(void);

$constructor_decls
$destructor_decls

static fi_prov_constructor_t *fi_prov_constructors[[]] = {
	${constructor_list}NULL
};

static fi_prov_destructor_t *fi_prov_destructors[[]] = {
	${destructor_list}NULL
};

#endif /* FI_STATIC_PROVIDERS_H */
EOF
])
		...
		[PROVIDERS_TO_BUILD="$PROVIDERS_TO_BUILD $1"
		 ...
		 AS_IF([test $$1_dl -eq 1],
			...
			[AC_MSG_NOTICE([$1 provider to be built statically])
			 PROVIDERS_STATIC="$1 $PROVIDERS_STATIC"])
		],
