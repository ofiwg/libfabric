...
static int sends_posted = 0;
static int sends_completed = 0;
...
static int run_client(void)
{
	...
	ret = (sends_completed == total_sends) ? 0 : -FI_EOTHER;
...
static int run_server(void)
{
	...
	return (recvs_completed == opts.iterations) ? 0 : -FI_EOTHER;
}
...
int main(int argc, char **argv)
{
	...
	while ((op = getopt_long(argc, argv, "h" ADDR_OPTS INFO_OPTS CS_OPTS API_OPTS,
		...
		case 'h':
			...
			FT_PRINT_OPTS_USAGE("-o <op>", "op: msg|senddata|write|writedata");
			return EXIT_FAILURE;
		}
	}
