...
/*
 * EFA device I/O descriptor formats.
 *
 * These are the layouts the EFA device itself reads and writes. They are
 * installed because a device-side FI_XPU kernel builds its own work requests
 * and reads its own completions, so it needs the same descriptor definitions
 * the provider's direct data path uses. The contents track the EFA kernel
 * driver's copy of this file and must be kept in step with it.
 */

#ifndef _RDMA_EFA_IO_DEFS_H_
#define _RDMA_EFA_IO_DEFS_H_

#include <stdint.h>

/*
 * Spelled out here rather than taken from a common header: this one is
 * installed, so it defines nothing a consumer has not asked for by name.
 */
#define EFA_IO_BIT(nr)		(1U << (nr))
#define EFA_IO_GENMASK(h, l)	(((1U << ((h) - (l) + 1)) - 1) << (l))
...
/* tx_meta_desc */
#define EFA_IO_TX_META_DESC_OP_TYPE_MASK	EFA_IO_GENMASK(3, 0)
#define EFA_IO_TX_META_DESC_HAS_IMM_MASK	EFA_IO_BIT(4)
#define EFA_IO_TX_META_DESC_INLINE_MSG_MASK	EFA_IO_BIT(5)
#define EFA_IO_TX_META_DESC_META_EXTENSION_MASK EFA_IO_BIT(6)
#define EFA_IO_TX_META_DESC_META_DESC_MASK	EFA_IO_BIT(7)
#define EFA_IO_TX_META_DESC_PHASE_MASK		EFA_IO_BIT(0)
#define EFA_IO_TX_META_DESC_FIRST_MASK		EFA_IO_BIT(2)
#define EFA_IO_TX_META_DESC_LAST_MASK		EFA_IO_BIT(3)
#define EFA_IO_TX_META_DESC_COMP_REQ_MASK	EFA_IO_BIT(4)
#define EFA_IO_TX_META_DESC_PROCESSING_HINTS_MASK            EFA_IO_GENMASK(1, 0)

/* tx_buf_desc */
#define EFA_IO_TX_BUF_DESC_LKEY_MASK EFA_IO_GENMASK(23, 0)

/* rx_desc */
#define EFA_IO_RX_DESC_LKEY_MASK  EFA_IO_GENMASK(23, 0)
#define EFA_IO_RX_DESC_FIRST_MASK EFA_IO_BIT(30)
#define EFA_IO_RX_DESC_LAST_MASK  EFA_IO_BIT(31)

/* cdesc_common */
#define EFA_IO_CDESC_COMMON_PHASE_MASK	     EFA_IO_BIT(0)
#define EFA_IO_CDESC_COMMON_Q_TYPE_MASK	     EFA_IO_GENMASK(2, 1)
#define EFA_IO_CDESC_COMMON_HAS_IMM_MASK     EFA_IO_BIT(3)
#define EFA_IO_CDESC_COMMON_OP_TYPE_MASK     EFA_IO_GENMASK(6, 4)
#define EFA_IO_CDESC_COMMON_UNSOLICITED_MASK EFA_IO_BIT(7)

#endif /* _RDMA_EFA_IO_DEFS_H_ */
