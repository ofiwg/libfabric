...
*FI_MORE*
  ...
  immediately be posted to the same queue after the current call returns.  Use
  of this flag may improve performance by enabling the provider to optimize
  ...
  Atomic operations place work on the endpoint's transmit queue, shared with
  sends, tagged sends, and RMA operations.  Work deferred under FI_MORE on the
  transmit queue is initiated when a subsequent transmit operation without
  FI_MORE is posted, or when the queue is flushed explicitly with fi_tx_flush
  (see [`fi_endpoint`(3)](fi_endpoint.3.html)).
