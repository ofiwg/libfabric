...
: Return the size of the endpoint's transmit context. See the `size` field of
  `tx_attr` (`fi_tx_attr`) for the full description. The value is limited by the
  endpoint's configuration and the provider's capabilities, and is less than or
  equal to `tx_attr->size`. This option only applies to the fi_getopt call.
  Providers that don't support this option will return -FI_ENOPROTOOPT. In that
  case, `tx_attr->size` should be used.

- *FI_OPT_RX_SIZE - size_t*
: Return the size of the endpoint's receive context. See the `size` field of
  `rx_attr` (`fi_rx_attr`) for the full description. The value is limited by the
  endpoint's configuration and the provider's capabilities, and is less than or
  equal to `rx_attr->size`. This option only applies to the fi_getopt call.
  Providers that don't support this option will return -FI_ENOPROTOOPT. In that
