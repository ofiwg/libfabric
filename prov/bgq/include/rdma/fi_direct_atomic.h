...
//fprintf(stderr, "%s:%s():%d buf = %p, count = %zu, datatype = %u, op = %u, key = %lu, addr = %p, is_fetch = %u, is_compare = %u\n", __FILE__, __func__, __LINE__, buf, count, datatype, op, key, (void*)addr, is_fetch, is_compare);
//if (is_fetch) fprintf(stderr, "%s:%s():%d fetch_vaddr = %p\n", __FILE__, __func__, __LINE__, fetch_vaddr);
//if (is_compare) fprintf(stderr, "%s:%s():%d compare_vaddr = %p\n", __FILE__, __func__, __LINE__, compare_vaddr);
//if (buf) { int32_t * foo = (int32_t *)buf; fprintf(stderr, "%s:%s():%d (int32_t *)buf = { %d %d %d %d %d %d %d %d %d %d }\n", __FILE__, __func__, __LINE__, foo[0], foo[1], foo[2], foo[3], foo[4], foo[5], foo[6], foo[7], foo[8], foo[9]); }
...
//err:
//	fi_bgq_unlock_if_required(&bgq_ep->lock, lock_required);
//	return ret;
}
