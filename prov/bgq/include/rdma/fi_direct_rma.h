...
#if 0
static inline void fi_bgq_mu_initialize_completion_descriptor (struct fi_bgq_ep * bgq_ep,
		MUHWI_Descriptor_t * desc,
		const uint64_t inc_paddr,
		const uint64_t dec_paddr)
{
	assert((inc_paddr != 0) || (dec_paddr != 0));

	/* copy the 'local completion' dput descriptor model */
	assert((((uintptr_t)&bgq_ep->tx.local_completion_model)&0x1F) == 0);
	assert((((uintptr_t)desc)&0x1F) == 0);
	DECLARE_FP_REGISTER(0);
	DECLARE_FP_REGISTER(1);
	VECTOR_LOAD_NU((void*)&bgq_ep->tx.local_completion_model, 0, f0);
	VECTOR_LOAD_NU((void*)&bgq_ep->tx.local_completion_model, 32, f1);
	VECTOR_STORE_NU(desc, 0, f0);
	VECTOR_STORE_NU(desc, 32, f1);

	if (!dec_paddr) {	/* likely */

		/* source -> "load increment" of the inc_paddr memory */
		desc->Pa_Payload =
			MUSPI_GetAtomicAddress(inc_paddr,
				MUHWI_ATOMIC_OPCODE_LOAD_INCREMENT);

		/* destination -> throw it away */
		MUSPI_SetRecPayloadBaseAddressInfo(desc,
			FI_BGQ_MU_BAT_ID_COUNTER, 0);

	} else {

		/* source -> "load decrement" of the dec_paddr memory */
		desc->Pa_Payload =
			MUSPI_GetAtomicAddress(dec_paddr,
				MUHWI_ATOMIC_OPCODE_LOAD_DECREMENT);

		if (inc_paddr) {

			uint64_t * dec_vaddr = NULL;
			assert(0 == Kernel_Physical2Virtual((void *)dec_paddr, (void **)&dec_vaddr));
			assert(1 == *dec_vaddr);

			/* destination -> "store add" the value from the
			 * dec_paddr memory into the inc_paddr memory; requires
			 * the value in the dec_paddr memory to be '1' */
			MUSPI_SetRecPayloadBaseAddressInfo(desc,
		...
		const enum fi_mr_mode mr_mode)
{
	...
	if (mr_mode == FI_MR_SCALABLE) {	/* branch will compile out */
		...
		hdr->rma.ndesc = niov;
//fprintf(stderr, "%s:%s():%d .Rec_FIFO_Id = %u\n", __FILE__, __func__, __LINE__, desc->PacketHeader.messageUnitHeader.Packet_Types.Memory_FIFO.Rec_FIFO_Id);
