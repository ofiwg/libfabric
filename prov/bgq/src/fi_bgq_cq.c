		...
		struct fid_cq **cq, void *context)
{
...
#if 0
	/* allocate the completion queue entries and initial the byte counters */
	{
		union fi_cq_bgq_entry * memptr = NULL;
		size_t bytes = sizeof(union fi_cq_bgq_entry) * bgq_cq->size;
		...
		Kernel_MemoryRegion_t cnk_memregion;
		if (Kernel_CreateMemoryRegion(&cnk_memregion, (void *)memptr, bytes)) {
			goto err;
		}
		uint64_t base_paddr = (uint64_t)cnk_memregion.BasePa + ((uint64_t)memptr - (uint64_t)cnk_memregion.BaseVa);

		unsigned n;
		for (n=0; n<bgq_cq->size; ++n) {
			assert(((uint64_t)&memptr[n].recv.byte_counter.value & 0x7) == 0);

			memptr[n].recv.byte_counter.value = 0;
			uint64_t offset = (uint64_t)&memptr[n].recv.byte_counter.value - (uint64_t)memptr;

			assert(((base_paddr+offset) & 0x07) == 0);
			memptr[n].recv.byte_counter.muhwi_paddr_rsh3b = (base_paddr+offset) >> 3;
		}
	...
	flight_recorder_init(&bgq_cq->flight_recorder);	
