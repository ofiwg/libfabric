...
//#define FI_BGQ_MU_RECFIFO_BYTES (0x01 << 20)	/* 1 MB */
...
//fprintf(stderr, "%s:%s():%d\n", __FILE__, __func__, __LINE__);
		...
		if (rc) goto err;
//fprintf(stderr, "%s:%s():%d requested_subgroup=%u, free_fifo_num=%u, free_fifo_ids[0]=%u, free_fifo_ids[1]=%u\n", __FILE__, __func__, __LINE__, requested_subgroup, free_fifo_num, free_fifo_ids[0], free_fifo_ids[1]);
...
//fprintf(stderr, "%s:%s():%d\n", __FILE__, __func__, __LINE__);
	...
	//bgq_domain->tx.rget_fifo_base = subgroup_offset;
	...
	*domain_attr = *fi_bgq_global.default_domain_attr;
	...
	if (hints) {
		...
		if (hints->domain) {
			...
			//domain_attr->cq_cnt		= ;	/* TODO - calculate this dynamic value? */
			//domain_attr->ep_cnt		= ;
...
//		caps & FI_MSG) {
		...
		FI_LOG(fi_bgq_global.prov, FI_LOG_DEBUG, FI_LOG_DOMAIN,
			...
			caps & FI_ATOMIC) {
		if (av_type == FI_AV_MAP) {
			name = FI_BGQ_DOMAIN_RMA_AVMAP_STR;
			type = FI_BGQ_DOMAIN_RMA_AVMAP;
		}
		else if (av_type == FI_AV_TABLE) {
			name = FI_BGQ_DOMAIN_RMA_AVTABLE_STR;
			type = FI_BGQ_DOMAIN_RMA_AVTABLE;
		}
		else {
			FI_LOG(fi_bgq_global.prov, FI_LOG_DEBUG, FI_LOG_DOMAIN,
					"unknown av_type %d\n", av_type);
...
//		domain_attr->mr_key_size = 0;
//		domain_attr->cq_data_size = 0;//FI_BGQ_RMA_CQ_DATA_SIZE;
//		break;
	default:
		FI_LOG(fi_bgq_global.prov, FI_LOG_DEBUG, FI_LOG_DOMAIN,
				"unknown error\n");
...
static int fi_bgq_domain_str_to_type(char *s, enum fi_bgq_domain_type *type)
{
	...
	*type = FI_BGQ_DOMAIN_UNSPEC;
	...
	if (s) {
//		if (!strncmp(s, FI_BGQ_DOMAIN_RMA_AVTABLE_STR,
//					strlen(FI_BGQ_DOMAIN_RMA_AVTABLE_STR))) {
//			*type = FI_BGQ_DOMAIN_RMA_AVTABLE;
//		} else if (!strncmp(s, FI_BGQ_DOMAIN_RMA_AVMAP_STR,
//					strlen(FI_BGQ_DOMAIN_RMA_AVMAP_STR))) {
//			*type = FI_BGQ_DOMAIN_RMA_AVMAP;
//		} else
