...
static int fi_bgq_ep_rx_init(struct fi_bgq_ep *bgq_ep)
{
...

	{
		...
		/* the *only* difference between a 'tagged' and 'non-tagged'
		 * send is the 'Rec_FIFO_Id' field in the mu packet header
		 *
		 * rendezvous multi-receive acks are only valid for
		 * 'non-tagged' message transfers
		 */
		desc->PacketHeader.messageUnitHeader.Packet_Types.Memory_FIFO.Rec_FIFO_Id =
			bgq_ep->rx.self.rx * BGQ_MU_NUM_REC_FIFOS_PER_SUBGROUP + 1;
