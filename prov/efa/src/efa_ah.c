...
/**
 * @brief initialize a pre-allocated base efa_ah object for a GID
 *
 * Create the ibv_ah for the GID, populate the base fields (ahn, gid, refcnt)
 * and register the AH in the per-domain hash map. Operates on a struct the
 * caller has already allocated (either a base efa_ah or the base portion of a
 * wrapping struct efa_rdm_ah); on failure it tears down any resource it created
 * but does not free the struct, which the caller owns.
 *
 * On ENOMEM from ibv_create_ah this helper simply fails with errno preserved;
 * the RDM layer (efa_rdm_ah_alloc) is responsible for evicting an implicit-only
 * AH and retrying, since eviction is an RDM-only policy.
 *
 * @param[in]	ah	pre-allocated AH to initialize
 * @param[in]	domain	efa_domain
 * @param[in]	gid	GID
 * @return	0 on success, a negative errno on failure
 */
int efa_ah_base_construct(struct efa_ah *ah, struct efa_domain *domain,
			  const uint8_t *gid)
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	ah->ibv_ah = ibv_create_ah(ibv_pd, &ibv_ah_attr);
	if (!ah->ibv_ah) {
		if (errno == EINVAL)
	...
	err = efadv_query_ah(ah->ibv_ah, &efa_ah_attr, sizeof(efa_ah_attr));
	if (err) {
		errno = err;
		EFA_WARN(FI_LOG_AV, "efadv_query_ah failed! err: %d\n", err);
		ibv_destroy_ah(ah->ibv_ah);
		return -err;
	}

	ah->refcnt = 1;
	ah->ahn = efa_ah_attr.ahn;
	memcpy(ah->gid, gid, EFA_GID_LEN);
	HASH_ADD(hh, domain->ah_map, gid, EFA_GID_LEN, ah);
	return 0;
}
 ...
 * shared reference count is incremented; otherwise a base efa_ah is allocated
 * and initialized via efa_ah_base_construct.
 *
 * @param[in]	domain	efa_domain
 * @param[in]	gid	GID
 * @return	the AH on success, NULL on failure
 */
	...
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	if (efa_ah_base_construct(efa_ah, domain, gid)) {
		free(efa_ah);
		return NULL;
	}
