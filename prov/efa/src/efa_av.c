 ...
 * @param[in]   implicit_av	whether to look in the implicit AV
 * @return	On success, return fi_addr to the peer who send the packet
 * 		If no such peer exist, return FI_ADDR_NOTAVAIL
 */
...
void efa_conn_rdm_deinit(struct efa_av *av, struct efa_conn *conn)
{
	...
	dlist_foreach_safe(&av->util_av.ep_list, entry, tmp) {
		...
		peer = efa_rdm_ep_peer_map_lookup(&ep->fi_addr_to_peer_map, conn->fi_addr);
		if (peer) {
			efa_rdm_peer_destruct(peer, ep);
			efa_rdm_ep_peer_map_remove(&ep->fi_addr_to_peer_map, conn->fi_addr);
		}
	}
}
 ...
 * @param[in]	implicit_av	whether insert address to implicit AV
...
struct efa_conn *efa_conn_alloc(struct efa_av *av, struct efa_ep_addr *raw_addr,
				uint64_t flags, void *context, bool insert_shm_av, bool implicit_av)
{
	...
	implicit_av ? av->used_implicit++ : av->used_explicit++;
...
int efa_av_insert_one(struct efa_av *av, struct efa_ep_addr *addr,
		      ...
		      bool insert_shm_av, bool implicit_av)
{
	struct efa_conn *conn;
	...
	if (implicit_fi_addr != FI_ADDR_NOTAVAIL) {
		...
		/* Grab the srx_lock here because the CQ read path could be
		 * using the peer and reverse AV maps */
	...
	conn = efa_conn_alloc(av, addr, flags, context, insert_shm_av, implicit_av);
	if (!conn) {
		*fi_addr = FI_ADDR_NOTAVAIL;
		ret = -FI_EADDRNOTAVAIL;
		goto out;
