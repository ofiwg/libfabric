 ...
 * the endpoint type. Only efa-direct uses wide-wqe; for the non-direct
...
{
	size_t max_inline_size = attr->inline_buf_size;

	if (ep_type == FI_EP_DGRAM) {
		*msg_inject_size_out = attr->inline_buf_size;
	...
	if (efa_device_support_wide_wqe())
		max_inline_size = attr->inline_buf_size_ex;
#endif

	*msg_inject_size_out = max_inline_size;
	/* RMA additionally requires the RDMA_WRITE device cap. */
	*rma_inject_size_out = efa_device_support_rdma_write() ? max_inline_size : 0;
}
			  ...
			  void *context)
{
	...
	efa_base_ep_get_inject_sizes(info->ep_attr->type, attr,
