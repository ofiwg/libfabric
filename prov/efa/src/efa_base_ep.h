...
struct efa_comp_action_wr {
	 ...
	 * action writes. Widened to match the efadv setter and the WQE field;
	 * the device truncates it to the action's entry_size.
	 */
 ...
 * flags are the EFADV_INLINE_DATA_ATTR_* bits describing the QP whose inline
 * size is queried: a QP that carries completion actions with data has a smaller
 * inline region, because the action block occupies part of the WQE. Returns the
 * max inline size (>= 0) or a negative errno (-FI_ENOSYS on builds without
 * efadv completion-action support). Shared by QP creation (to clamp the
 ...
 * EFADV_SQ_DEPTH_ATTR_COMP_ACTION_WITH_DATA for actions); max_inline_data is the inline
...
struct efa_base_ep {
	...
	/* Whether completion-action support is enabled on this endpoint
	 * (via FI_OPT_EFA_COMP_ACTION). Must be set before the endpoint is
