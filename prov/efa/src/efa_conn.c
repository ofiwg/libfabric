 ...
 */
int efa_rdm_av_entry_insert_shm_av(struct efa_av *av, struct efa_rdm_av_entry *av_entry)
{
 ...
 */
void efa_rdm_av_entry_deinit(struct efa_av *av, struct efa_rdm_av_entry *av_entry)
{
	int err;
...
static void efa_av_entry_remove_from_util_av(struct efa_av_array *entry_map, struct util_av *util_av,
	...
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	efa_av_entry->ah = efa_ah_alloc(av->domain, raw_addr->raw, false);
	if (!efa_av_entry->ah)
	...
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	efa_av_entry_remove_from_util_av(av->addr_to_entry_map, &av->util_av, entry, fi_addr);
}
 ...
 */
struct efa_av_entry *efa_rdm_av_entry_alloc_explicit(struct efa_av *av, struct efa_ep_addr *raw_addr,
 ...
 */
struct efa_rdm_av_entry *efa_rdm_av_entry_alloc_implicit(struct efa_av *av, struct efa_ep_addr *raw_addr,
 ...
 */
void efa_rdm_av_entry_release_explicit(struct efa_av *av, struct efa_av_entry *entry)
	OFI_TSA_REQUIRES(efa_util_av_lock_sym)
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	assert(ofi_genlock_held(&av->util_av.lock));
	...
	if (av->domain->info_type == EFA_INFO_RDM)
		efa_rdm_av_entry_deinit(av, container_of(entry, struct efa_rdm_av_entry, efa_av_entry));
	...
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	dlist_remove(&av_entry->ah_implicit_conn_list_entry);
	efa_ah_release(av->domain, av_entry->efa_av_entry.ah, true);
	efa_av_entry_remove_from_util_av(av->addr_to_entry_map_implicit, &av->util_av_implicit,
 ...
 */
void efa_rdm_av_entry_release_implicit_ah_unsafe(struct efa_av *av, struct efa_rdm_av_entry *av_entry)
	OFI_TSA_REQUIRES(efa_implicit_av_lock_sym)
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	assert(EFA_GENLOCK_HELD(&av->util_av_implicit.lock, efa_implicit_av_lock_sym));
