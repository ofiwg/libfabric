...
/*
 * Provider object backing a public completion-action handle. The public struct
 * is the first member so it is recovered with container_of; the efadv handle
 * owns the kernel resource. The application owns the lifetime of the object
 * (create/close), so no provider-side bookkeeping is needed.
 */
struct efa_comp_action {
	struct fid_efa_comp_action comp_action;
	struct efadv_comp_action *efadv_action;
};
#endif
