...
static void efa_env_unregistered_param_get(void)
{
	...
	tmp = getenv("FI_EFA_ENABLE_COMP_ACTION");
	if (tmp)
		efa_env.enable_comp_action = atoi(tmp);
}
...
void efa_env_param_get(void)
{
	...
	fi_param_get_size_t(&efa_prov, "rdm_txe_pool_size", &efa_env.rdm_txe_pool_size);
...
void efa_env_define()
{
	...
	fi_param_define(&efa_prov, "rdm_txe_pool_size", FI_PARAM_SIZE_T,
