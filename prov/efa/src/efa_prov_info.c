				  ...
				  enum fi_ep_type ep_type)
{
...
#if HAVE_INLINE_BUF_SIZE_EX
	{
		...
		/*
		 * efadv_get_max_sq_depth returns negative errno on failure
		 * (e.g. old kernel/firmware that doesn't support the query).
		 * Fall back to device attribute in that case.
		 */
