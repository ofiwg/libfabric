				   ...
				   struct efa_rdm_av_entry *entry)
{
	...
	assert(ofi_genlock_held(&((struct efa_rdm_domain *) av->efa_av.domain)->srx_lock));
...
static void efa_rdm_ah_lru_move(struct efa_domain *domain, struct efa_ah *ah)
{
	...
	dlist_insert_tail(&rdm_ah->lru_list_entry, &rdm_domain->ah_lru_list);
}
...
static int efa_rdm_ah_evict(struct efa_domain *domain)
{
	...
	assert(ofi_genlock_held(&rdm_domain->srx_lock));
			       ...
			       uint64_t flags, void *context)
{
	...
	ofi_genlock_lock(&((struct efa_rdm_domain *) av->efa_av.domain)->srx_lock);
	...
	ofi_genlock_unlock(&((struct efa_rdm_domain *) av->efa_av.domain)->srx_lock);
