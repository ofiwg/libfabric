...
void efa_rdm_ep_set_use_zcpy_rx(struct efa_rdm_ep *ep)
{
	...
	if (ep->base_ep.util_ep.caps & unsupported_caps) {
		EFA_INFO(FI_LOG_EP_CTRL, "Unsupported capabilities, disable zero-copy receive protocol\n");
		ep->use_zcpy_rx = false;
		goto out;
	}
	...
	if (ep->max_msg_size > ep->mtu_size - ep->user_info->ep_attr->msg_prefix_size) {
		EFA_INFO(FI_LOG_EP_CTRL, "max_msg_size %lu is greater than the mtu size limit: %lu, turn off zero-copy receive protocol\n",
	...
	if (efa_rdm_ep_need_sas(ep)) {
		EFA_INFO(FI_LOG_EP_CTRL, "FI_ORDER_SAS is requested, disable zero-copy receive protocol\n");
		ep->use_zcpy_rx = false;
		goto out;
	}
