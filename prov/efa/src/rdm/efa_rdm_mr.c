...
static int efa_rdm_mr_dereg_impl(struct efa_rdm_mr *efa_rdm_mr)
{
	...
	if (efa_rdm_mr->efa_mr.peer.iface == FI_HMEM_CUDA &&
	    (efa_rdm_mr->efa_mr.peer.flags & OFI_HMEM_DATA_DEV_REG_HANDLE)) {
		assert(efa_rdm_mr->efa_mr.peer.hmem_data);
		int err = ofi_hmem_dev_unregister(FI_HMEM_CUDA, (uint64_t)efa_rdm_mr->efa_mr.peer.hmem_data);
		if (err) {
			EFA_WARN(FI_LOG_MR, "Unable to de-register cuda handle\n");
			ret = err;
		}
		efa_rdm_mr->efa_mr.peer.hmem_data = NULL;
	}
	...
	if (mr_attr->iface == FI_HMEM_CUDA) {
		...
		if (!(flags & FI_MR_DMABUF) && cuda_is_gdrcopy_enabled()) {
			...
			int err = ofi_hmem_dev_register(FI_HMEM_CUDA, mr_iov.iov_base, mr_iov.iov_len,
							(uint64_t *)&efa_rdm_mr->efa_mr.peer.hmem_data);
			efa_rdm_mr->efa_mr.peer.flags |= OFI_HMEM_DATA_DEV_REG_HANDLE;
			if (err) {
				EFA_WARN(FI_LOG_MR,
			      ...
			      uint64_t flags, struct fid_mr **mr_fid)
{
	...
	if (domain->shm_domain) {
		uint64_t shm_flags = efa_rdm_mr->efa_mr.peer.flags;
		struct fi_mr_attr shm_attr = mr_attr;

		if (mr_attr.iface != FI_HMEM_SYSTEM)
			shm_flags |= FI_HMEM_DEVICE_ONLY;

		shm_attr.hmem_data = efa_rdm_mr->efa_mr.peer.hmem_data;
