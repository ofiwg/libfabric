			     ...
			     uint64_t tag, uint32_t op, uint64_t flags)
{
	...
	EFA_DBG(FI_LOG_EP_DATA,
		...
		peer->av_entry->fi_addr, txe->total_len, tag, op, flags, txe->msg_id);
			...
			uint64_t flags)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		...
		efa_addr = msg->addr;
		shm_msg->addr = peer->av_entry->shm_fi_addr;
		      ...
		      void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		...
		return fi_sendv(efa_rdm_ep->shm_ep, iov, shm_desc, count, peer->av_entry->shm_fi_addr, context);
	}
		     ...
		     void *desc, fi_addr_t dest_addr, void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		...
		return fi_send(efa_rdm_ep->shm_ep, buf, len, desc? shm_desc[0] : NULL, peer->av_entry->shm_fi_addr, context);
	}
			 ...
			 void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		if (desc)
			efa_rdm_get_desc_for_shm(1, &desc, shm_desc);
		return fi_senddata(efa_rdm_ep->shm_ep, buf, len, desc? shm_desc[0] : NULL, data, peer->av_entry->shm_fi_addr, context);
	}
		       ...
		       fi_addr_t dest_addr)
{
	...
	assert(peer);
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		return fi_inject(efa_rdm_ep->shm_ep, buf, len, peer->av_entry->shm_fi_addr);
	}
			   ...
			   fi_addr_t dest_addr)
{
	...
	assert(peer);
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		return fi_injectdata(efa_rdm_ep->shm_ep, buf, len, data, peer->av_entry->shm_fi_addr);
	}
			 ...
			 uint64_t flags)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		...
		efa_addr = tmsg->addr;
		shm_tmsg->addr = peer->av_entry->shm_fi_addr;
		       ...
		       uint64_t tag, void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		if (desc)
			efa_rdm_get_desc_for_shm(count, desc, shm_desc);
		return fi_tsendv(efa_rdm_ep->shm_ep, iov, desc? shm_desc : NULL, count, peer->av_entry->shm_fi_addr, tag, context);
	}
		      ...
		      void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		if (desc)
			efa_rdm_get_desc_for_shm(1, &desc, shm_desc);
		return fi_tsend(efa_rdm_ep->shm_ep, buf, len, desc? shm_desc[0] : NULL, peer->av_entry->shm_fi_addr, tag, context);
	}
			  ...
			  uint64_t tag, void *context)
{
	...
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		if (desc)
			efa_rdm_get_desc_for_shm(1, &desc, shm_desc);
		return fi_tsenddata(efa_rdm_ep->shm_ep, buf, len, desc? shm_desc[0] : NULL, data, peer->av_entry->shm_fi_addr, tag, context);
	}
			...
			fi_addr_t dest_addr, uint64_t tag)
{
	...
	assert(peer);
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		return fi_tinject(efa_rdm_ep->shm_ep, buf, len, peer->av_entry->shm_fi_addr, tag);
	}
			    ...
			    uint64_t data, fi_addr_t dest_addr, uint64_t tag)
{
	...
	assert(peer);
	if (peer->is_local && efa_rdm_ep->shm_ep) {
		return fi_tinjectdata(efa_rdm_ep->shm_ep, buf, len, data, peer->av_entry->shm_fi_addr, tag);
	}
