...
void efa_rdm_rxe_report_completion(struct efa_rdm_ope *rxe)
{
	...
	if (OFI_UNLIKELY(rxe->cq_entry.len < rxe->total_len)) {
		EFA_WARN(FI_LOG_CQ,
			 ...
			 rxe->peer->av_entry->efa_av_entry.fi_addr, rxe->peer->av_entry->implicit_fi_addr, rxe->rx_id, rxe->msg_id, rxe->cq_entry.tag,
	...
	if (!(rxe->internal_flags & EFA_RDM_RXE_RECV_CANCEL) &&
		...
		EFA_DBG(FI_LOG_CQ,
			...
			"\n",
			rxe->peer->av_entry->efa_av_entry.fi_addr,
		...
		efa_rdm_tracepoint(recv_end,
			    ...
			    rxe->total_len, rxe->cq_entry.tag, rxe->peer->av_entry->efa_av_entry.fi_addr);
...
void efa_rdm_txe_report_completion(struct efa_rdm_ope *txe)
{
	...
	if (efa_rdm_txe_should_update_cq(txe)) {
		EFA_DBG(FI_LOG_CQ,
		       ...
		       PRIu64 "\n",
		       txe->peer->av_entry->efa_av_entry.fi_addr, txe->tx_id, txe->msg_id,
		       txe->cq_entry.tag, txe->total_len);

	efa_rdm_tracepoint(send_end,
		    txe->msg_id, (size_t) txe->cq_entry.op_context,
		    txe->total_len, txe->cq_entry.tag, txe->peer->av_entry->efa_av_entry.fi_addr);
