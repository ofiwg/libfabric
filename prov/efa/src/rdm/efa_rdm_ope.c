...
inline int efa_rdm_ope_process_queued_ope(struct efa_rdm_ope *ope, uint16_t flag)
{
	int ret = 0;

	assert(flag & EFA_EDM_OPE_QUEUED_FLAGS);
	...
	if (OFI_UNLIKELY(ret)) {
		...
		if (ope->type == EFA_RDM_TXE)
			efa_rdm_txe_handle_error(ope, -ret, flag == EFA_RDM_OPE_QUEUED_RNR ? FI_EFA_ERR_PKT_SEND : FI_EFA_ERR_PKT_POST);
		else
			efa_rdm_rxe_handle_error(ope, -ret, flag == EFA_RDM_OPE_QUEUED_RNR ? FI_EFA_ERR_PKT_SEND : FI_EFA_ERR_PKT_POST);
		return ret;
	}
