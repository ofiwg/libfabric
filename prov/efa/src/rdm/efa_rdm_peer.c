	...
	if (ofi_recvwin_is_exp(robuf, msg_id))
		...
		if (ofi_recvwin_id_processed(robuf, msg_id)) {
			...
			if (OFI_UNLIKELY(!efa_env.rx_copy_ooo)) {
				/* We didn't copy the pke to the OOO pool, so
				 * this pke is from the RX entry pool. Because
				 * this pke may not be processed during this
				 * cq_read call, this packet is considered a
				 * "held" packet. And we have to increment
				 * efa_rx_pkts_held */
				assert(ooo_entry->alloc_type ==
...
void efa_rdm_peer_move_overflow_pke_to_recvwin(struct efa_rdm_peer *peer)
{
	...
	dlist_foreach_container_safe (
		...
		if (OFI_UNLIKELY(!efa_env.rx_copy_ooo)) {
			assert(overflow_pkt_entry->alloc_type ==
			       EFA_RDM_PKE_FROM_EFA_RX_POOL);
			overflow_pkt_entry->ep->efa_rx_pkts_held--;
		}
