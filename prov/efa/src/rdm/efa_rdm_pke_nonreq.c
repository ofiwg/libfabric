					...
					struct efa_rdm_ope *ope)
{
	...
	if (ope->type == EFA_RDM_RXE) {
		...
		assert(ope->type == EFA_RDM_TXE);
		/*
		 * SKIP (REF_MSG_ID_SKIP): the receiver owes no completion, so
		 * the packet only unblocks its reorder window past msg_id.
		 * True for EAGER (a single REQ that never landed) and for
		 * medium / runt-only runtread that acked zero bytes (delivered
		 * nothing). A msg_id protocol that delivered partial data owes
		 * a FI_ECANCELED on the matched rxe (REF_MSG_ID); everything
		 * else (LONGCTS) carries the receiver's rxe index
		 * (REF_OPE_INDEX). bytes_acked is final here: emission only
		 * happens once every data WR has drained.
		 */
		if (efa_rdm_pkt_type_is_eager_rtm(ope->protocol) ||
		    (efa_rdm_txe_peer_abort_uses_msg_id(ope) &&
		     ope->bytes_acked == 0)) {
