 ...
 * dangling pke->ope pointers that a later flush would dereference. The
 * entries are dropped without performing the copies: an aborted rxe must
 * not copy into a buffer it is about to error out, and an unmatched rxe
 * has no user buffer at all.
 *
