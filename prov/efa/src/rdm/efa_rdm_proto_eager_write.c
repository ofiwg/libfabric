 ...
 */
static void efa_rdm_pke_proc_eager_rtw(struct efa_rdm_pke *pkt_entry,
 ...
 * negative libfabric error code on failure
 */
static ssize_t efa_rdm_pke_init_dc_eager_rtw(struct efa_rdm_pke *pkt_entry,
				      struct efa_rdm_ope *txe)
{
						...
						uint64_t *pke_send_flags)
{
	...
	*pke_send_flags = (txe->fi_flags & FI_MORE) ? FI_MORE : 0;
	...
	if (txe->req_pkt_type == efa_rdm_proto_eager_write.req_pkt_type_dc)
		ret = efa_rdm_pke_init_dc_eager_rtw(pkt_entry, txe);
	else
		ret = efa_rdm_pke_init_eager_rtw(pkt_entry, txe);
	if (ret)
		goto err;
