 ...
 *
 * Long CTS is the fallback protocol that can always be used. Returns
 ...
 *
 * Tracks bytes_acked and reports completion when all data has been
 * acknowledged.
 */
 ...
 *
 * Only releases the TXE when both all send completions and the receipt
 * have been received.
 */
...
/* TODO: Implement TX completions for pke when implementing the RX path */
 ...
 *
 * Sends a single RTM packet containing the first chunk of data. The
 * receiver will respond with a CTS packet to request the remaining data.
 *
					    ...
					    struct efa_rdm_ope *txe)
{
	...
	EFA_DBG(FI_LOG_EP_DATA,
		"longcts protocol: dc_requested=%d tagged=%d\n",
		delivery_complete_requested, tagged);
	...
	EFA_INFO(FI_LOG_EP_DATA,
		"longcts protocol: posting %d pke(s), total_len %lu, msg_id %" PRIu32 "\n",
