 ...
 *
 * Returns true if the message size is within the medium threshold
 * (default 64KB for system memory).
 */
 ...
 *
 * Tracks bytes_acked and only reports completion and releases the TXE
 * when all packets of the message have been acknowledged.
 */
 ...
 * and the receipt packet has been received.
 */
 ...
 *
 * Splits the message data across multiple packet entries, balancing
 * data sizes for performance and respecting memory alignment. Each
 * packet carries a segment offset and total message length in its header.
 *
 * On success, ep->send_pkt_entry_vec contains the packet entries and
 * ep->send_pkt_entry_vec_size is set to the number of packets.
 *
					   ...
					   struct efa_rdm_ope *txe)
{
	...
	size_t *pkt_entry_data_size_vec = ep->send_pkt_entry_data_sizes;
	...
	EFA_DBG(FI_LOG_EP_DATA,
		"medium protocol: total_len %lu, max_data_per_pkt %d\n",
		txe->total_len, single_pkt_entry_max_data_size);
	 ...
	 * single packet size
	 */
	single_pkt_entry_data_size = (txe->total_len - 1) / pkt_entry_cnt + 1;
	...
	EFA_INFO(FI_LOG_EP_DATA,
		"medium protocol: posting %d pkes, total_len %lu, msg_id %" PRIu32 "\n",
