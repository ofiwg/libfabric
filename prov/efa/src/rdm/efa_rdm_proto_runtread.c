 ...
 * meeting the minimum read size, and peer RDMA read support. Also
 * requires a non-zero runt size.
 */
 ...
 *
 * Simply releases the TX packet entry since the remaining data transfer
							...
							struct efa_rdm_ope *txe)
{
	...
	EFA_DBG(FI_LOG_EP_DATA,
		"runtread protocol: runt_size %lu, total_len %lu\n",
		txe->bytes_runt, txe->total_len);
 ...
 *
 * Splits the runt portion across multiple packets (similar to medium)
 * while including RDMA read IOVs in each packet header so the receiver
 * can fetch the remaining data.
 *
					     ...
					     struct efa_rdm_ope *txe)
{
	...
	int i, j, ret, req_pkt_type, iface, available_tx_pkts,
	...
	for (i = 0; i < pkt_entry_cnt; ++i) {
		...
		for (j = 0; j < txe->iov_count; ++j) {
			read_iov[j].addr = (uint64_t) txe->iov[j].iov_base;
			read_iov[j].len = txe->iov[j].iov_len;
			read_iov[j].key = fi_mr_key(txe->desc[j]);
		}
		payload_offset =
	...
	EFA_INFO(FI_LOG_EP_DATA,
		"runtread protocol: posting %d pkes, runt_size %lu, total_len %lu\n",
		pkt_entry_cnt, txe->bytes_runt, txe->total_len);
