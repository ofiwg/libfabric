...
ssize_t efa_rdm_rma_post_write(struct efa_rdm_ep *ep, struct efa_rdm_ope *txe)
{
	...
	if (efa_rdm_rma_should_write_using_rdma(ep, txe, txe->peer)) {
		 ...
		 * imm not consume an rx buffer on the responder side. Older libfabric
		 * or new libfabric working with old driver expects an rx buffer to be
		 * consumed for the rdma write with imm operation.
		 * As a result, we cannot post an rdma-write with imm when initiator
		 * supports unsolicited write recv while peer doesn't.
		 */
		if ((txe->fi_flags & FI_REMOTE_CQ_DATA) &&
