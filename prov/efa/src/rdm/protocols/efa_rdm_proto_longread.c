...
struct efa_rdm_proto efa_rdm_proto_longread = {
	.name = "longread",
	.wants_mr = true,
	.can_use_protocol = &efa_rdm_proto_longread_can_use_for_send,
					    ...
					    uint64_t *pke_send_flags)
{
	...
	/*
	 * The single RTM packet this protocol posts is what tells the receiver
	 * to start reading, so the doorbell is not worth deferring even though
	 * the packet count would allow it: ring it immediately rather than
	 * honoring the caller's FI_MORE.
	 */
	*pke_send_flags = 0;
