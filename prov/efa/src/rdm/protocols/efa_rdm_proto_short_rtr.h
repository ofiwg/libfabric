/* Copyright Amazon.com, Inc. or its affiliates. All rights reserved. */
/* SPDX-License-Identifier: BSD-2-Clause OR GPL-2.0-only */

#ifndef _EFA_RDM_PROTO_SHORT_RTR_H
#define _EFA_RDM_PROTO_SHORT_RTR_H

#include "efa_rdm_proto.h"

extern struct efa_rdm_proto efa_rdm_proto_short_rtr;

int efa_rdm_proto_short_rtr_construct_tx_pkes(struct efa_rdm_ep *ep,
					      struct efa_rdm_peer *peer,
					      const struct fi_msg *msg,
					      uint32_t op, uint64_t tag,
					      uint64_t flags,
					      uint32_t internal_flags,
					      struct efa_rdm_ope *txe,
					      uint64_t *pke_send_flags);

ssize_t efa_rdm_proto_short_rtr_handle_send_completion(
	struct efa_rdm_pke *pkt_entry);

#endif /* _EFA_RDM_PROTO_SHORT_RTR_H */
