...
void test_impl_cq_read_empty_cq(struct efa_resource *resource, enum fi_ep_type ep_type)
{
	...
	if (ep_type == FI_EP_DGRAM) {
		...
		ibv_cqx = container_of(efa_dgram_ep->base_ep.util_ep.rx_cq, struct efa_cq, util_cq)->ibv_cq_ex;
	} else {
...
/**
 * @brief test CQ's fi_cq_read() works properly when rdma-core return success
 * status.
 */
void test_efa_cq_read_success(struct efa_resource *resource,
				     fi_addr_t *addr,
				     int ibv_wc_opcode)
{
	int ret;
	...
	if (ibv_wc_opcode == IBV_WC_RECV) {
		ibv_cqx = container_of(base_ep->util_ep.rx_cq, struct efa_cq, util_cq)->ibv_cq_ex;
		ibv_cqx->start_poll = &efa_mock_ibv_start_poll_return_mock;
		ibv_cqx->wr_id = (uintptr_t)12345;
		will_return(efa_mock_ibv_start_poll_return_mock, IBV_WC_SUCCESS);
	} else {
		ibv_cqx = container_of(base_ep->util_ep.tx_cq, struct efa_cq, util_cq)->ibv_cq_ex;
		/* this mock will set ibv_cq_ex->wr_id to the wr_id of the head of global send_wr,
		* and set ibv_cq_ex->status to mock value */
		ibv_cqx->start_poll = &efa_mock_ibv_start_poll_use_saved_send_wr_with_mock_status;
		will_return(efa_mock_ibv_start_poll_use_saved_send_wr_with_mock_status, IBV_WC_SUCCESS);
	}
	...
	ibv_cqx->read_qp_num = &efa_mock_ibv_read_qp_num_return_mock;
	will_return(efa_mock_ibv_end_poll_check_mock, NULL);
	will_return_always(efa_mock_ibv_next_poll_return_mock, 0);
	will_return_always(efa_mock_ibv_read_opcode_return_mock, ibv_wc_opcode);
	will_return_always(efa_mock_ibv_read_qp_num_return_mock, base_ep->qp->qp_num);
	will_return_maybe(efa_mock_ibv_read_vendor_err_return_mock, 0);
#if HAVE_EFADV_CQ_EX
	...
	will_return_always(efa_mock_ibv_read_byte_len_return_mock, 4096);
...
void test_efa_cq_read_send_success(struct efa_resource **state)
{
	...
	test_efa_cq_read_success(resource, &addr, IBV_WC_SEND);
	efa_unit_test_buff_construct(&send_buff, resource, 4096 /* buff_size */);

	assert_int_equal(g_ibv_submitted_wr_id_cnt, 0);
	ret = fi_send(resource->ep, send_buff.buff, send_buff.size, fi_mr_desc(send_buff.mr), addr, (void*)12345);
	assert_int_equal(ret, 0);
	assert_int_equal(g_ibv_submitted_wr_id_cnt, 1);
...
void test_efa_cq_read_recv_success(struct efa_resource **state)
{
	...
	test_efa_cq_read_success(resource, &addr, IBV_WC_RECV);
	efa_unit_test_buff_construct(&recv_buff, resource, 4096 /* buff_size */);

	ret = fi_recv(resource->ep, recv_buff.buff, recv_buff.size, fi_mr_desc(recv_buff.mr), addr, NULL);
	assert_int_equal(ret, 0);
