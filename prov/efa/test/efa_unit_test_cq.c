			     ...
			     bool is_unsolicited_write_recv)
{
	...
	if (efa_env.track_mr && ctx) {
		...
		direct_ope = ofi_buf_alloc(ep->base_ep.ope_pool);
		assert_non_null(direct_ope);
		direct_ope->context = ctx;
		dlist_insert_tail(&direct_ope->entry, &ep->base_ep.ope_list);
	}
...
void test_efa_cq_read_mixed_success_error(struct efa_resource **state)
{
	...
	if (efa_env.track_mr) {
		...
		direct_ope1 = ofi_buf_alloc(ep->base_ep.ope_pool);
		assert_non_null(direct_ope1);
		direct_ope1->context = efa_context1;
		dlist_insert_tail(&direct_ope1->entry, &ep->base_ep.ope_list);

		direct_ope2 = ofi_buf_alloc(ep->base_ep.ope_pool);
		assert_non_null(direct_ope2);
		direct_ope2->context = efa_context2;
		dlist_insert_tail(&direct_ope2->entry, &ep->base_ep.ope_list);

		direct_ope3 = ofi_buf_alloc(ep->base_ep.ope_pool);
		assert_non_null(direct_ope3);
		direct_ope3->context = efa_context3;
		dlist_insert_tail(&direct_ope3->entry, &ep->base_ep.ope_list);
	}
