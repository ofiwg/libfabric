...
/**
 * @brief Test that unsolicited write recv is disabled when FI_OPT_EFA_CQ_FLOW_CONTROL is set
 */
void test_efa_rdm_ep_setopt_cq_flow_control(struct efa_resource **state)
{
	...
	ep = container_of(resource->ep, struct efa_rdm_ep,
		base_ep.util_ep.ep_fid);
	assert_false(ep->base_ep.cq_flow_control);
	assert_int_equal(fi_setopt(&resource->ep->fid, FI_OPT_ENDPOINT,
				   FI_OPT_EFA_CQ_FLOW_CONTROL, &optval,
				   sizeof(optval)),
			 FI_SUCCESS);
	assert_true(ep->base_ep.cq_flow_control);
	assert_int_equal(fi_enable(resource->ep), 0);
	assert_false(ep->base_ep.qp->unsolicited_write_recv_enabled);
}

/**
 * @brief Test setting FI_OPT_EFA_CQ_FLOW_CONTROL will fail without FI_RX_CQ_DATA in efa direct
 */
...
/**
 * @brief Test setting FI_OPT_EFA_CQ_FLOW_CONTROL with FI_RX_CQ_DATA will disable unsolicited write recv
 */
void test_efa_direct_ep_setopt_cq_flow_control_with_rx_cq_data(struct efa_resource **state)
{
	...
	efa_base_ep = container_of(resource->ep, struct efa_base_ep, util_ep.ep_fid);
	assert_false(efa_base_ep->cq_flow_control);
	assert_int_equal(fi_setopt(&resource->ep->fid, FI_OPT_ENDPOINT,
				   FI_OPT_EFA_CQ_FLOW_CONTROL, &optval,
				   sizeof(optval)),
			 FI_SUCCESS);
	assert_true(efa_base_ep->cq_flow_control);
	assert_int_equal(fi_enable(resource->ep), 0);
	assert_false(efa_base_ep->qp->unsolicited_write_recv_enabled);
}
