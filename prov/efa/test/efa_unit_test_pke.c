...
{
	/* TODO: Once the RX path is implemented for Long CTS in the refactored
	 * path, re-implement this test
	 *
	 * This test is testing how the RX path responds to a READ NACK packet
	 * by constructing a long CTS RTM packet and how the TX completion for
	 * that long CTS RTM packet is handled.
	 *
	 * It is not possible to implement this test correctly when only the TX
	 * path of long CTS is implemented in the refactored path.
	 */
}
