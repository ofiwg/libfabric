...
/**
 * @brief A read NACK long CTS REQ carries no payload, and its send completion
 * must neither account for it nor touch the txe.
 *
 * The long CTS protocol moved to the refactored code path, so the packet is
 * built by efa_rdm_proto_longcts_construct_tx_pkes() -- which keys off
 * EFA_RDM_OPE_READ_NACK to leave the payload out and set the matching wire flag
 * -- and completed by the callback the packet carries. That callback has to
 * return before dereferencing the txe: the CTSDATA packets that finish the
 * message can complete first and release it.
 */
void test_efa_rdm_pke_handle_longcts_rtm_send_completion(void **state)
{
	...
	/* Build the fallback long CTS REQ the way the protocol does. */
	err = efa_rdm_proto_longcts_construct_tx_pkes(efa_rdm_ep, peer, NULL,
						     txe->op, txe->tag,
						     txe->fi_flags,
						     txe->internal_flags, txe);
	assert_int_equal(err, 0);
	...
	pkt_entry = efa_rdm_ep->send_pkt_entry_vec[0];
	assert_non_null(pkt_entry);
	assert_ptr_equal(pkt_entry->handle_pke,
			 &efa_rdm_proto_longcts_handle_rtm_send_completion);

	/* No payload: the runt packets already delivered the head. */
	assert_int_equal(pkt_entry->payload_size, 0);

	/*
	 * The wire flag is what makes the receiver reuse the rxe it already has
	 * for this msg_id instead of allocating a second one.
	 */
	...
	/* The callback owns the packet entry, so do not release it here. */
