 ...
 * the protocols, so the zero-copy protocol carries the rest end to end: the
 * per-peer dispatch that reaches it, the headerless packet its
 * construct_tx_pkes() builds, the user_recv_qp routing efa_rdm_pke_sendv()
 * derives from the packet flags, and a send completion that must reach the
 * protocol callback rather than the headerless arm of the pkt_type switch.
 */
...
void test_proto_longread_construct_pkes_single_pke(void **state)
{
	 ...
	 * from an operation it does not handle.
	 */
	assert_int_equal(txe->req_pkt_type, EFA_RDM_LONGREAD_MSGRTM_PKT);
...
void test_proto_longcts_construct_pkes_single_pke(void **state)
{
	 ...
	 * from an operation it does not handle.
	 */
	assert_int_equal(txe->req_pkt_type, EFA_RDM_LONGCTS_MSGRTM_PKT);
