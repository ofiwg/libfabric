...
/*
 * efa_rdm_ep_get_peer_implicit_unsafe requires its caller to hold the
 * util_domain and implicit AV locks, as the CQ read path does.
 */
static struct efa_rdm_peer *test_av_get_peer_implicit(struct efa_rdm_ep *ep,
						      fi_addr_t addr)
{
	struct efa_av *av = ep->base_ep.av;
	struct efa_rdm_av *rdm_av = (struct efa_rdm_av *) av;
...
/*
 * efa_rdm_av_reverse_lookup_implicit_unsafe requires its caller to hold the
 * util_domain and implicit AV locks, as the CQ read path does.
 */
static fi_addr_t test_av_reverse_lookup_implicit(struct efa_av *av, uint16_t ahn,
						 uint16_t qpn,
...
void test_av_reinsertion(void **state)
{
	...
	assert_int_equal(peer->av_entry->efa_av_entry.fi_addr, fi_addr);
	assert_int_equal(efa_is_same_addr(&raw_addr, efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)), 1);
...
void test_av_reverse_av_remove_qpn_collision(void **state)
{
	...
	assert_int_equal(efa_rdm_av_reverse_lookup(av, ahn, 100, NULL),
			 fi_addr1);
...
static struct efa_rdm_peer *test_av_get_peer_from_implicit_av(struct efa_resource *resource)
{
	...
	assert_int_equal(peer->av_entry->efa_av_entry.fi_addr, FI_ADDR_NOTAVAIL);
	assert_int_equal(efa_is_same_addr(&raw_addr, efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)), 1);

	test_addr = test_av_reverse_lookup_implicit(av, ahn, raw_addr.qpn, NULL);
...
void test_av_implicit_to_explicit(void **state)
{
	...
	implicit_fi_addr = peer->av_entry->implicit_fi_addr;
	assert_ptr_equal(efa_rdm_av_addr_to_entry_implicit(av, implicit_fi_addr),
			 peer->av_entry);
	...
	/* Insert explicitly */
	raw_addr.qpn = efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)->qpn;
	raw_addr.qkey = efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)->qkey;
	err = fi_av_insert(resource->av, &raw_addr, 1, &explicit_fi_addr, 0, NULL);
	assert_int_equal(err, 1);
	assert_null(efa_rdm_av_addr_to_entry_implicit(av, implicit_fi_addr));
	explicit_entry = efa_av_addr_to_entry(av, explicit_fi_addr);
	assert_non_null(explicit_entry);
	assert_ptr_equal(&peer->av_entry->efa_av_entry, explicit_entry);
	test_av_verify_av_hash_cnt(av, 1, 0, 0, 0);
	...
	assert_int_equal(peer->av_entry->efa_av_entry.fi_addr, explicit_fi_addr);
	assert_int_equal(peer->av_entry->implicit_fi_addr, FI_ADDR_NOTAVAIL);
	assert_int_equal(efa_is_same_addr(&raw_addr, efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)), 1);

	ahn = efa_rdm_ep->self_ah->ahn;
	test_addr = efa_rdm_av_reverse_lookup(av, ahn, raw_addr.qpn, NULL);
	...
	assert_int_equal(err, 0);
	assert_null(efa_av_addr_to_entry(av, explicit_fi_addr));
	test_av_verify_av_hash_cnt(av, 0, 0, 0, 0);
}

static void test_av_implicit_av_verify_lru_list_first_last_elements(
	struct efa_av *av, struct efa_rdm_av_entry *first_expected,
	struct efa_rdm_av_entry *last_expected)
{
	struct dlist_entry *first_entry, *last_entry;
	struct efa_rdm_av_entry *first_actual, *last_actual;

	first_entry = ((struct efa_rdm_av *)(av))->implicit_av_lru_list.next;
	last_entry = ((struct efa_rdm_av *)(av))->implicit_av_lru_list.prev;

	first_actual = container_of(first_entry, struct efa_rdm_av_entry,
					 implicit_av_lru_entry);
	last_actual = container_of(last_entry, struct efa_rdm_av_entry,
					implicit_av_lru_entry);

	assert_ptr_equal(first_actual, first_expected);
	assert_ptr_equal(last_actual, last_expected);
}
...
void test_av_implicit_av_lru_eviction(void **state)
{
	...
	((struct efa_rdm_av *)(av))->implicit_av_size = 2;
...
void test_ah_refcnt(void **state)
{
	...
	peer = test_av_get_peer_implicit(efa_rdm_ep, fi_addr);
...
void test_ah_lru_eviction_impl(bool explicit)
{
	...
	peer = test_av_get_peer_implicit(efa_rdm_ep[0], fi_addr);
...
void test_av_rdm_reverse_lookup_explicit(void **state)
{
	...
	lookup_addr = efa_rdm_av_reverse_lookup(av, ahn, 42, NULL);
	...
	lookup_addr = efa_rdm_av_reverse_lookup(av, ahn, 99, NULL);
	assert_int_equal(lookup_addr, FI_ADDR_NOTAVAIL);
...
void test_av_rdm_addr_to_entry_after_remove(void **state)
{
	...
	entry = (struct efa_rdm_av_entry *) efa_av_addr_to_entry(av, 0);
	assert_null(entry);
	...
	assert_non_null(entry->efa_av_entry.ah);
	assert_int_equal(entry->efa_av_entry.fi_addr, fi_addr);
	assert_int_equal(entry->implicit_fi_addr, FI_ADDR_NOTAVAIL);
	assert_int_equal(efa_av_entry_ep_addr(&entry->efa_av_entry)->qpn, 99);
	assert_int_equal(efa_av_entry_ep_addr(&entry->efa_av_entry)->qkey, 0x9999);
...
void test_av_rdm_insert_remove_with_peer(void **state)
{
	...
	assert_int_equal(peer->av_entry->efa_av_entry.fi_addr, fi_addr);
	assert_int_equal(peer->av_entry->implicit_fi_addr, FI_ADDR_NOTAVAIL);
	assert_int_equal(efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)->qpn, 55);
	assert_int_equal(efa_av_entry_ep_addr(&peer->av_entry->efa_av_entry)->qkey, 0x5555);
	assert_ptr_equal(peer->ep, efa_rdm_ep);

	/* Peer map lookup should find the same peer */
	peer2 = efa_rdm_ep_get_peer_explicit(efa_rdm_ep, fi_addr);
...
void test_av_implicit_to_explicit_peer_updated(void **state)
{
	...
	assert_int_equal(implicit_peer->av_entry->efa_av_entry.fi_addr, FI_ADDR_NOTAVAIL);
	assert_int_not_equal(implicit_fi_addr, FI_ADDR_NOTAVAIL);
	test_av_verify_av_hash_cnt(av, 0, 0, 1, 0);

	/* Remember the AH — it should be reused after migration */
	ah_before = implicit_peer->av_entry->efa_av_entry.ah;
	assert_non_null(ah_before);

	/* Now insert explicitly with the same address */
	struct efa_ep_addr raw_addr;
	memcpy(&raw_addr, implicit_peer->av_entry->efa_av_entry.ep_addr, EFA_EP_ADDR_LEN);
	...
	explicit_peer = efa_rdm_ep_get_peer_explicit(efa_rdm_ep, explicit_fi_addr);
	assert_non_null(explicit_peer);
	assert_ptr_equal(explicit_peer, implicit_peer);
	assert_int_equal(explicit_peer->av_entry->efa_av_entry.fi_addr, explicit_fi_addr);
	...
	assert_ptr_equal(explicit_peer->av_entry->efa_av_entry.ah, ah_before);
...
void test_av_rdm_batch_insert(void **state)
{
	...
	for (i = 0; i < 3; i++) {
		entry = (struct efa_rdm_av_entry *) efa_av_addr_to_entry(av, fi_addrs[i]);
		assert_non_null(entry);
		assert_non_null(entry->efa_av_entry.ah);
		assert_int_equal(entry->efa_av_entry.fi_addr, fi_addrs[i]);
		assert_int_equal(efa_av_entry_ep_addr(&entry->efa_av_entry)->qpn, 10 + i);
		assert_int_equal(efa_av_entry_ep_addr(&entry->efa_av_entry)->qkey, 0x1000 + i);
	}

	/* Remove one at a time and verify counts */
	fi_av_remove(resource->av, &fi_addrs[0], 1, 0);
	test_av_verify_av_hash_cnt(av, 2, 0, 0, 0);
	assert_null(efa_av_addr_to_entry(av, fi_addrs[0]));
...
void test_av_rdm_prv_reverse_av(void **state)
{
	...
	/* Verify first entry */
	entry1 = (struct efa_rdm_av_entry *) efa_av_addr_to_entry(av, fi_addr1);
	assert_non_null(entry1);
	assert_int_equal(efa_av_entry_ep_addr(&entry1->efa_av_entry)->qkey, 0xAAAA);

	/* Reverse lookup should find first entry */
	lookup_addr = efa_rdm_av_reverse_lookup(av, ahn, 20, NULL);
	...
	/* Verify second entry */
	entry2 = (struct efa_rdm_av_entry *) efa_av_addr_to_entry(av, fi_addr2);
	assert_non_null(entry2);
	assert_int_equal(efa_av_entry_ep_addr(&entry2->efa_av_entry)->qkey, 0xBBBB);

	/* Both entries should share the same AH (same GID) */
	assert_ptr_equal(entry1->efa_av_entry.ah, entry2->efa_av_entry.ah);
...
void test_av_implicit_av_unbounded(void **state)
{
	...
	((struct efa_rdm_av *)(av))->implicit_av_size = 0;

	for (i = 0; i < num_peers; i++)
		test_av_get_peer_from_implicit_av(resource);

	/* All peers should still be in the implicit AV */
	assert_int_equal(HASH_CNT(hh, ((struct efa_rdm_av *)(av))->util_av_implicit.hash), num_peers);
	/* No peer should have been evicted */
	assert_int_equal(HASH_CNT(hh, ((struct efa_rdm_av *)(av))->evicted_peers_hashset), 0);
}
...
/**
 * @brief efa_rdm_av_implicit_av_lru_move on a single-element list
 *
 * Insert exactly one implicit peer; the LRU list has exactly one node.
 * Call efa_rdm_av_implicit_av_lru_move on it — this exercises the
...
void test_av_implicit_av_lru_move_single(void **state)
{
	...
	efa_rdm_av_implicit_av_lru_move(av, peer->av_entry);
