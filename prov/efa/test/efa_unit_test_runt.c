...
/**
 * @brief Test the runt size returned by efa_rdm_ep_get_runt_size
 *
 * @param resource efa_resource
 * @param iface hmem iface
 * @param num_runt_bytes_in_flight how many runt bytes are in flight on the peer side
 ...
 */
static
void test_efa_rdm_ep_get_runt_size_impl(
		struct efa_resource *resource,
		enum fi_hmem_iface iface, size_t num_runt_bytes_in_flight,
		size_t total_runt_size, size_t total_len, size_t expected_runt_size)
{
	...
	efa_domain->num_runt_bytes_in_flight = num_runt_bytes_in_flight;
	...
	runt_size = efa_rdm_ep_get_runt_size(efa_rdm_ep, &mock_txe);
	assert_true(runt_size == expected_runt_size);
}

void test_efa_rdm_ep_get_runt_size_no_enough_runt(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
	size_t num_runt_bytes_in_flight;
	size_t total_runt_size;

	efa_unit_test_resource_construct(resource, FI_EP_RDM);

	msg_length = 12000;
	num_runt_bytes_in_flight = 1001;
	total_runt_size = 1000;
	/* 1001 is exceeding 1000, cannot runt */
	expected_runt_size = 0;
	test_efa_rdm_ep_get_runt_size_impl(resource, FI_HMEM_SYSTEM, num_runt_bytes_in_flight, total_runt_size, msg_length, expected_runt_size);
}

void test_efa_rdm_ep_get_runt_size_cuda_memory_smaller_than_alignment(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
	...
	msg_length = 12000;
	num_runt_bytes_in_flight = 1000;
	total_runt_size = 1048;
	/* 1048 - 1000 is smaller than cuda memory alignment (64), runt size must be 0 */
	expected_runt_size = 0;
	test_efa_rdm_ep_get_runt_size_impl(resource, FI_HMEM_CUDA, num_runt_bytes_in_flight, total_runt_size, msg_length, expected_runt_size);
}

void test_efa_rdm_ep_get_runt_size_cuda_memory_exceeding_total_len(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
...
void test_efa_rdm_ep_get_runt_size_cuda_memory_normal(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
	...
	msg_length = 12000;
	num_runt_bytes_in_flight = 10000;
	total_runt_size = 16384;
...
void test_efa_rdm_ep_get_runt_size_host_memory_smaller_than_alignment(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
...
void test_efa_rdm_ep_get_runt_size_host_memory_exceeding_total_len(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
...
void test_efa_rdm_ep_get_runt_size_host_memory_normal(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t expected_runt_size;
...
/**
 * @brief Test the protocol returned by efa_rdm_ep_select_readbase_rtm()
 *
 ...
 */
static
void test_efa_rdm_ep_select_readbase_rtm_impl(
		struct efa_resource *resource,
		enum fi_hmem_iface iface, size_t num_runt_bytes_in_flight,
		...
		int op, uint64_t fi_flags, int expected_protocol)
{
	...
	readbase_rtm = efa_rdm_ep_select_readbase_rtm(efa_rdm_ep, &mock_txe);
	assert_true(readbase_rtm == expected_protocol);
}

void test_efa_rdm_ep_select_readbase_rtm_no_runt(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t num_runt_bytes_in_flight;
	...
	test_efa_rdm_ep_select_readbase_rtm_impl(resource, FI_HMEM_CUDA, num_runt_bytes_in_flight,
	total_runt_size, msg_length, ofi_op_msg, 0, EFA_RDM_LONGREAD_MSGRTM_PKT);
}

void test_efa_rdm_ep_select_readbase_rtm_do_runt(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	size_t msg_length;
	size_t num_runt_bytes_in_flight;
