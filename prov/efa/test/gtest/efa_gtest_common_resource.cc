...
static struct fi_info *efa_test_alloc_hints(enum fi_ep_type ep_type, const char *fabric_name)
{
				 ...
				 enum fi_ep_type ep_type, const char *fabric_name)
{
	...
	resource->hints = efa_test_alloc_hints(ep_type, fabric_name);
	ASSERT_NE(resource->hints, nullptr);

	fi_version = (!strcmp(EFA_DIRECT_FABRIC_NAME, fabric_name))
			     ? FI_VERSION(2, 0)
			     : FI_VERSION(1, 14);

	ret = fi_getinfo(fi_version, NULL, NULL, 0ULL, resource->hints, &resource->info);
