...
/**
 * @brief The efa_test_device_supports_* helpers read the selected device list,
 * which the provider only populates on the first fi_getinfo. Probe with a
 * throwaway one so an answer does not depend on whether an earlier test
 * happened to run.
 */
static void probe_selected_device()
{
...
static bool device_supports_rdma_read()
{
	if (!efa_test_device_supports_rdma_read())
		probe_selected_device();
	return efa_test_device_supports_rdma_read();
}

static bool device_supports_rdma_write()
{
	if (!efa_test_device_supports_rdma_write())
		probe_selected_device();
	return efa_test_device_supports_rdma_write();
}

/* efa-direct advertises FI_RMA only when the device has both. */
static bool device_supports_rma()
{
	return device_supports_rdma_read() && device_supports_rdma_write();
}
...
class EfaFiMoreTest : public TestWithParam<EfaFiMoreParam>
{
	...
	void SetUp() override
	{
		...
		uint64_t mr_access = FI_SEND | FI_RECV;
		struct fi_info *hints = efa_test_alloc_default_hints(
			FI_EP_RDM, EFA_DIRECT_FABRIC_NAME);
		...
		if (op() != EFA_TEST_POST_SEND) {
			...
			/* A device without RDMA support rejects a registration
			 * asking for RMA access outright, and the send case has
			 * no use for it. */
			mr_access |= FI_READ | FI_WRITE;
		}
		...
		int ret = fi_mr_reg(resource.domain, buf, kBufSize, mr_access,
				    0, 0, 0, &mr, NULL);
 ...
 * be tested here.
 *
 * Only the RMA cases need a device that can do RMA, so they gate themselves
 * rather than the fixture: the send cases are worth running on the hardware
 * that cannot.
 */
class EfaFiMoreRdmTest : public TestWithParam<int>
{
	...
	void SetUp() override
	{
		...
		ASSERT_NE(buf, nullptr);
		/* Asking for access the device cannot provide fails the
		 * registration outright, so ask for what it has. The tests
		 * needing more than that skip themselves. */
		uint64_t mr_access = FI_SEND | FI_RECV;
		if (device_supports_rdma_read())
			mr_access |= FI_READ;
		if (device_supports_rdma_write())
			mr_access |= FI_WRITE;
		int ret = fi_mr_reg(resource.domain, buf, kRdmBufSize, mr_access,
				    0, 0, 0, &mr, NULL);
		ASSERT_EQ(ret, 0) << "fi_mr_reg failed: " << fi_strerror(-ret);
		desc = fi_mr_desc(mr);
...
{
	if (!device_supports_rdma_read())
		GTEST_SKIP() << "device does not support RDMA read";
...
{
	if (!device_supports_rdma_write())
		GTEST_SKIP() << "device does not support RDMA write";
